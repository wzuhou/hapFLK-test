...
library(phangorn)
args <- commandArgs(T)
###phangorn:::reorderPruning was not supported
...
#D=read.table("hapflk_reynolds.txt",row.names=1)
D=read.table(args[1],row.names=1)
#F=kinship(D,outgroup="Barnev")
F=kinship(D,outgroup="")
write.table(F$matrix,paste(args[1],"hapmap_kinship_noroot.txt",sep="_"),quote=FALSE,col.names=FALSE)
